...
BT::RosNodeParams withDefaultActionName(const BT::RosNodeParams& params,
...
class CalibrateGripper : public BT::RosActionNode<gigatino_msgs::action::Calibrate>
{
public:
  using Action = gigatino_msgs::action::Calibrate;
  ...
  CalibrateGripper(const std::string& name, const NodeConfig& config,
                   const RosNodeParams& params)
    : Base(name, config, withDefaultActionName(params, "gigatino/calibrate"))
  {
  }
  ...
  {
    RCLCPP_INFO(logger(), "%s sending calibration goal", name().c_str());
    return true;
  }
...
CreateRosNodePlugin(robotino_behavior_tree::CalibrateGripper, "CalibrateGripper");
