...
class MoveToShelf : public BT::RosActionNode<motor_move_msgs::action::MoveToShelf>
{
public:
  ...
  MoveToShelf(const std::string& name, const NodeConfig& config, const RosNodeParams& params)
    : Base(name, config, withDefaultActionName(params, "move_to_shelf_action"))
  {
  }
  ...
  static BT::PortsList providedPorts()
  {
    return providedBasicPorts({
        BT::InputPort<double>("timeout", 10.0, "Maximum move-to-shelf time in seconds"),
        BT::InputPort<double>("speed", 0.1, "Approach speed"),
  ...
  {
    goal.timeout = static_cast<float>(getInput<double>("timeout").value_or(10.0));
    goal.speed = static_cast<float>(getInput<double>("speed").value_or(0.1));

    RCLCPP_INFO(logger(), "%s sending goal: timeout=%.3f speed=%.3f", name().c_str(),
                goal.timeout, goal.speed);
    return true;
  }
...
CreateRosNodePlugin(robotino_behavior_tree::MoveToShelf, "MoveToShelf");
